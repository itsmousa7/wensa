...
void main() {
  // The paths must match the routes registered in router_provider.dart
  // ('/placeDetails', '/eventDetails') — a shared link is a deep link.
  test('placeShareUrl builds a placeId query url', () {
    expect(placeShareUrl('abc'), 'https://wensa.app/placeDetails?placeId=abc');
  });

  test('eventShareUrl builds an eventId query url', () {
    expect(eventShareUrl('e1'), 'https://wensa.app/eventDetails?eventId=e1');
  });
  ...
  test('placeShareCaption (en) includes name and url', () {
    ...
    expect(
      c,
      'Check out Cafe X on Wensa!\nhttps://wensa.app/placeDetails?placeId=abc',
    );
  });
  ...
  test('placeShareCaption (ar) includes name and url', () {
    ...
    expect(c, contains('https://wensa.app/placeDetails?placeId=abc'));
  });
  ...
  test('eventShareCaption (en) includes name and url', () {
    ...
    expect(
      c,
      'Check out Show on Wensa!\nhttps://wensa.app/eventDetails?eventId=e1',
    );
  });
